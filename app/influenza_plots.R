...
####################################################################################################
# Influenza Wasterwater Data for Alberta
####################################################################################################
...
plot_data[is.finite(inf_a_mean_ma),p_value:=rank(inf_a_mean_ma)/sum(ifelse(is.finite(inf_a_mean_ma),1,0)),by=c("location")]
plot_data[inf_a_mean<1e-4,inf_a_mean:=NA]
...
p <- ggplot(plot_data[date>=Sys.Date()-90 & !grepl("WT Plant",location),],aes(x=date))+
  geom_col(aes(y=inf_a_mean_ma,fill=p_value))+
  geom_step(aes(y=inf_a_mean_ma),colour="black",lwd=0.8)+
  ...
  scale_y_continuous("Influenza A")+
  xlab(NULL)+
  scale_fill_viridis_c(option = "viridis",name="Percentile")+
  scale_color_discrete(name=NULL)+
  scale_shape_discrete(name=NULL)+
  coord_cartesian(ylim=c(0,NA))+
  labs(title="Alberta Influenza A Wastewater Trends",
...
ggsave(filename = paste0("output/ab_influenza_a.png"),plot = p, width=8,height=8,units = "in",dpi=150)

# Influenza B
plot_data[is.finite(inf_b_mean_ma),p_value:=rank(inf_b_mean_ma)/sum(ifelse(is.finite(inf_b_mean_ma),1,0)),by=c("location")]
plot_data[inf_b_mean<1e-4,inf_b_mean:=NA]

plot_data[,max_date:=max(date),by="location"]
plot_data[,last_perc:=mean(ifelse(date==max_date,p_value,NA),na.rm=T),by=c("location")]
plot_data[,location_label:=paste0(location," (P",round(last_perc*100,0),", ", strftime(max_date,"%b %e") ,")")]

if(max(plot_data$inf_b_mean,na.rm=T)>0){

  p <- ggplot(plot_data[date>=Sys.Date()-90 & !grepl("WT Plant",location),],aes(x=date))+
    geom_col(aes(y=inf_b_mean_ma,fill=p_value))+
    geom_step(aes(y=inf_b_mean_ma),colour="black",lwd=0.8)+
    facet_wrap(location_label~.,scales="free_y")+
    theme_bw()+
    theme(axis.text.y=element_blank(),axis.ticks.y=element_blank(),legend.position = 'bottom',
          strip.text = element_text(size=rel(0.6)))+
    scale_y_continuous("Influenza B")+
    xlab(NULL)+
    scale_fill_viridis_c(option = "viridis",name="Percentile")+
    scale_color_discrete(name=NULL)+
    scale_shape_discrete(name=NULL)+
    coord_cartesian(ylim=c(0,NA))+
    labs(title="Alberta Influenza B Wastewater Trends",
         subtitle=paste0("Latest sample as of ",max(plot_data$date)),
         caption="Percentile (PXX) indicates fraction of days below the most recent value for each location\nData Source: Centre for Health Informatics, Cumming School of Medicine, University of Calgary")

  ggsave(filename = paste0("output/ab_influenza_b.png"),plot = p, width=8,height=8,units = "in",dpi=150)
}